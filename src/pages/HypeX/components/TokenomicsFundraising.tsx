const TokenomicsFundraising = () => {
  ...
  return (
    <div className="flex flex-col">
      <h2 className="mb-12 text-xl font-bold text-center text-white md:text-6xl">
        Tokenomics - Fundraising
      </h2>

      <div className="p-4 border md:p-8 rounded-xl">
        <div className="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                  ...
                  <tr>
                    {tableCols.map((col, index) => (
                      <th
                        key={index}
                        scope="col"
                        className={`md:px-6 md:py-3 md:text-lg text-sm font-medium tracking-wider text-center uppercase ${
                  ...
                  {tableData.map((item) => (
                    <tr>
                      {item.map((cell, index) => (
                        <td
                          key={index}
                          className={`md:px-6 py-4 px-3 md:py-4 md:text-lg text-sm text-center text-white whitespace-nowrap ${
            ...
            <button
              type="button"
              className="w-full p-3 text-sm font-bold text-center text-white rounded-full md:p-5 md:text-xl bg-darkBtn"
            >
              Total Sum : 7,350,000$
            </button>
          </div>
